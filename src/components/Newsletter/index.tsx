    ...
    <section className="p-4 md:p-0 min-h-[300px] h-full w-full bg-white rounded-3xl my-6 flex items-center justify-center flex-col gap-6">
      ...
      <form onSubmit={handleSubmit(submitForm)} className="flex gap-2 md:gap-6">
        <div className="relative">
          <input type="email" {...register("email")} placeholder="Infome seu e-mail" className="max-w-[360px] w-full h-10 px-4 border border-zinc-500 rounded-md" />
          {errors.email && <p className="text-red-600 text-xs"> {errors.email.message} </p>}
        </div>
        <button type="submit" className="rounded-md h-10 px-4 flex items-center justify-center bg-primary-500 transition-all duration-300 ease-linear text-white text-base hover:brightness-150">
          Enviar
        </button>
      </form>
    </section>
